  ...
  convertCanvasToTsplBinary(
    ...
    for (let y = 0; y < targetHeightDots; y++) {
      for (let xByte = 0; xByte < widthBytes; xByte++) {
        ...
        // Inverting each generated byte with byte = byte ^ 0xFF ensures WHITE canvas remains unheated white paper
        // and black text, barcodes, and lines heat the thermal head.
        const finalByte = invertPolarity ? (byteVal ^ 0xFF) : byteVal;
